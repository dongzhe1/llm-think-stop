...
- [Risk-controlled comparison](#risk-controlled-comparison)
- [Extended baselines and robustness checks](#extended-baselines-and-robustness-checks)
...
#### `analyze_risk_controlled.py`

**UCB risk-controlled comparison across all settings.** For each of 18 task-model settings, selects a stop threshold on the calibration split using Hoeffding's UCB finite-grid correction (guaranteeing test risk ≤ α), then evaluates on the held-out test split with paired bootstrap confidence intervals. Compares LearnStop-8 (8-feature logistic classifier) against four scalar baselines and a BestScalar oracle that applies simultaneous correction across all scalar policies. Runs CPU-only from saved `raw.npz` archives.

```bash
# Sequential
python scripts/analyze_risk_controlled.py

# Parallel (recommended — each setting is independent)
python scripts/analyze_risk_controlled.py --jobs 8
python scripts/analyze_risk_controlled.py --jobs -1   # all cores
```

| Flag | Default | Description |
|------|---------|-------------|
| `--jobs` | `1` | Number of parallel workers; `-1` = all cores |

Key design choices:

- **40/60 cal/test split**, seed 123; LearnStop-8 trained via grouped 5-fold CV on all N questions (no separate train split needed)
- **UCB** = R̂ + √(log(K/δ) / 2n_cal), where K = grid size per policy; **BestScalar** uses K = Σ K_p (simultaneous correction)
- **Confidence-leap** converted to a single score `max(0, Δconf) × conf` for single-threshold compatibility
- **Bootstrap** B = 5000, seed 20270207, paired question-level resamples; thresholds frozen before test
- **Cost model**: KV-fork — `think_cost + n_probes × ans_tokens (default 48)`

Output directory: `results/risk_controlled_comparison/`

| Output file | Content |
|-------------|---------|
| `results/learnstop_variant_audit.csv` | Per-setting audit confirming which variant (LS-8 vs LS-10) matches the reported paper values |
| `results/risk_controlled_by_policy_with_confleap.csv` | 576 rows: 18 settings × 4 α levels × 8 policies |
| `results/risk_controlled_deltas_vs_bestscalar_with_confleap.csv` | Paired deltas (LearnStop vs BestScalar) per setting and α |
| `results/risk_controlled_alpha015_main_table_with_ci.tex` | LaTeX Table 2 at α = 0.15 |
| `results/compact18_summary_with_confleap.csv` | 18-setting summary (peak frontier gain + risk-controlled save% + verdict) |
| `results/sensitivity_curve.csv` | Subset sensitivity (50–800 questions × 50 repeats × 4 policies) |
| `results/failure_cases_small_table.csv` | Archetypal per-question failure cases (LearnStop wins / Scalar wins / Lost-correct / Overhead failure) |
| `run_manifest.yaml` | Full reproducibility manifest |
| `sanity_checks.txt` | Automated sanity checks (feasibility, candidate sets, CI method) |

---

#### `analyze_extended_baselines.py`

**Extended proxy baselines and robustness checks.** Runs three additional experiments and four documentation audits on top of the core risk-controlled comparison. CPU-only, imports from `analyze_risk_controlled.py`.

```bash
python scripts/analyze_extended_baselines.py
python scripts/analyze_extended_baselines.py --jobs 6
```

| Flag | Default | Description |
|------|---------|-------------|
| `--jobs` | `1` | Number of parallel workers for per-setting jobs |

**Experiments:**

| Experiment | Description |
|-----------|-------------|
| **Extended proxy baselines** | Adds DEER-style (`conf × mkr`), PUMA-style (`run_len × (1 − mkr/mkr_max)`), EAT-style (`−ent_var`) as single-threshold UCB policies; 3 settings × 4 α levels |
| **Calibration split sensitivity** | Repeats the risk-controlled comparison across 5 different cal/test split seeds (123, 456, 789, 1011, 2025); 6 primary Qwen3 settings |
| **Failure cases with answer evolution** | For each category (LearnStop wins / Scalar wins / Lost-correct / Overhead failure / Oscillating answer), finds the most informative example and records per-checkpoint answer, confidence, entropy, marker density, and run length |

**Documentation audits (output to `logs/`):**

| Item | Output |
|------|--------|
| H100 profiling protocol | `results/h100_profile_protocol.yaml` — batch size, ans cap, and latency data used in Figure 3 |
| Capped-token wording | `logs/capped_token_wording_audit.txt` — lines in the paper that refer to total-token savings without explicitly noting the 48-token cap |
| Risk correction formula | `results/risk_correction_manifest.yaml` — UCB formula, K values per policy, comparison of LearnStop vs BestScalar correction stringency |
| Supplement table | `results/supplement_reproducibility_table.tex` — LaTeX table of reproducibility-critical protocol details |

Output directory: `results/extended_baselines/`
...
│   ├── analyze_risk_controlled.py     #   UCB risk-controlled comparison (18 settings, paired CIs)
│   ├── analyze_extended_baselines.py  #   Extended proxy baselines + robustness checks (CPU-only)
